...
public class ProjectileWeapon : Weapon
{
    ...
    {
        if (currentAttackInterval > 0) return true;
    ...
    protected override bool Attack(int attackCount = 1)
    {
        ...
        {
            Debug.LogWarning(string.Format("Weapon {0} tried to attack, but no projectile prefab is assigned!", name));
            ActivateCooldown();
            return false;
        }
        ...
        // Do we have more attacks to do in this sequence?
        if (attackCount > 0)
        {
    ...
    // rotates the facing od the point by spawnAngle
