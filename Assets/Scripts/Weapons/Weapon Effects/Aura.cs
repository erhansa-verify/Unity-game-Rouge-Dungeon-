...
/// <summary>
/// An aura is a damage-over-time effect that applies to a specific area in time intervals.
...
public class Aura : WeaponEffect
{
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        if (other.TryGetComponent(out EnemyStats es))
        {
            ...
            {
                // Always start with an interval of 0, so that it will get
                // damaged in the nect Update() tick.
    ...
    void OnTriggerExit2D(Collider2D other)
    {
        if (other.TryGetComponent(out EnemyStats es))
        {
            // Do not direcly remove the target upon leaving,
            // because we still have to track thier cooldowns.
            if (affectedTargets.ContainsKey(es))
            {
