...
public class UIUpgradeWindow : MonoBehaviour
{
    ...
    // These are the paths to the different UI elements in the upgradeOptionTemplates.
    [Header("Paths")]
    ...
    // of different things in the UpgradeWindow.
    ...
    // possible upgrades to show. It will select <pick> number of upgrades and show
    ...
    public void SetUpgrades(PlayerInventory inventory, List<ItemData> possibleUpgrades, int pick = 3, string tooltip = "")
    {
        ...
        // Activate only the number of upgrade options we need, and are the buttons and the
        ...
        foreach (RectTransform r in upgradeOptions)
        {
            if (activeOptions < pick && activeOptions < totalPossibleUpgrades)
            {
                ...
                // insert the buttin action binding.
        ...
        // sizes all the elements so they dont  exceed the size of the box
        RecalculateLayout();
    }
    ...
    {
        // Calculates the total available height for all options,
        // then divides it by the number of options.
